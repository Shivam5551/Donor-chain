...
export const Appbar = () => {
    ...
    return (
        <div className="fixed p-3 z-50 bg-white border-b border-gray-200 flex h-fit w-full items-center justify-between">
            ...
            <span className="flex hover:cursor-pointer items-center gap-2">
                ...
                <Link to="/" className="text-base text-black lg:text-2xl font-medium">
                    DonorChain
                </Link>
            </span>
            ...
            <div
                ref={menuRef}
                className={`absolute lg:static top-16 right-4 lg:right-0 bg-white lg:bg-transparent text-black rounded-xl lg:rounded-none shadow-lg lg:shadow-none flex flex-col lg:flex-row gap-3.5 font-semibold p-4 lg:p-0 lg:flex lg:items-center transition-all duration-300 ${isOpen ? "flex" : "hidden lg:flex"}`}
            >

                <Tspan path="/" title="Home" />
                <Tspan path={'/dashboard/org'} title="Dashboard" />
                <Tspan path="/discover" title="Discover Charities" />
                <Tspan path="/about" title="About" />

                {walletAddress ? (
                    <div className="flex items-center bg-gray-200 rounded-2xl hover:!cursor-not-allowed gap-2 w-full lg:w-fit">
                        <button disabled={true} className="cursor-not-allowed text-base lg:text-xl w-full lg:w-fit flex justify-center items-center bg-slate-200 text-center lg:bg-transparent hover:bg-slate-200 lg:text-black hover:text-black text-black p-3 rounded-2xl transition-all duration-300 hover:tracking-wide hover:opacity-80">
                            Connected
                        </button>
                    </div>
                ) : (
                    <button
                        ...
                        className={`px-4 py-2 inline-flex justify-center items-center text-black text-base lg:text-2xl rounded-lg transition-all duration-200 ${isConnecting ? "bg-gray-500 cursor-not-allowed" : "bg-orange-600 hover:bg-orange-700"
                            }`}
                    >
                        <div className="flex items-center justify-center shadow-2xs bg-transparent rounded-full p-1">
                ...
                {userLogin ? (isOpen ? <Button onClick={handleLogout} bg="bg-red-500 text-white hover:bg-red-600" >Log out</Button> : <LogoutButton />) : <Button onClick={handleLogin} bg="bg-green-500 hover:bg-green-800">Login</Button>}
            </div>
...
const Button = ({ onClick, bg, children }: { onClick: () => void; bg: string; children: React.ReactNode }) => (
    <button onClick={onClick} className={`${bg} inline-flex justify-center text-base lg:text-2xl items-center hover:rounded-3xl transition-all ease-in duration-400 p-2.5 rounded-xl`}>
        {children}
    </button>
);

const Tspan = ({ path, title }: { path: string; title: string }) => (
    <Link to={path} className="hover:cursor-pointer text-base lg:text-xl flex justify-center items-center bg-slate-200 lg:bg-transparent hover:bg-slate-200 lg:text-black hover:text-black text-black p-3 rounded-2xl transition-all duration-300 hover:tracking-wide hover:opacity-80">
        {title}
    </Link>
);
